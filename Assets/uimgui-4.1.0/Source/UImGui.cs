...
namespace UImGui
{
	...
	public class UImGui : MonoBehaviour
	{
		...
		[SerializeField]
		public ShaderResourcesAsset _shaders = null;
		...
		private void OnEnable()
		{
			...
			{
				//_renderCommandBuffer.SetRenderTarget(SettingGUIManager.Instance.tex2);
		...
		{
			if (!SettingGUIManager.Instance || !SettingGUIManager.Instance.tex2) return;
			UImGuiUtility.SetCurrentContext(_context);
			ImGuiIOPtr io = ImGui.GetIO();

			Constants.PrepareFrameMarker.Begin(this);
			_context.TextureManager.PrepareFrame(io);
			_platform.PrepareFrame(io, /*_camera.pixelRect*/new Rect(0, 0, Screen.width, Screen.height));
			ImGui.NewFrame();
			...
			_renderCommandBuffer.SetRenderTarget(SettingGUIManager.Instance.tex2);
			_renderCommandBuffer.ClearRenderTarget(true, true, Color.clear);
