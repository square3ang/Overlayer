	...
	internal class TextureManager
	{
		...
		public void BuildFontAtlas(ImGuiIOPtr io, in FontAtlasConfigAsset settings, FontInitializerEvent custom)
		{
			...
			if (settings == null)
			{
				...
				OverlayerSettings.FontInitializer(io);
			...
			for (int fontIndex = 0; fontIndex < settings.Fonts.Length; fontIndex++)
			{
				...
				string fontPath = System.IO.Path.Combine(Application.isEditor ? Application.streamingAssetsPath : UImGui.modFolder, fontDefinition.Path);
				...
				unsafe
				{
					...
					fontConfigPtr.GlyphRanges = /*AllocateGlyphRangeArray(fontDefinition.Config)*/io.Fonts.GetGlyphRangesKorean();
