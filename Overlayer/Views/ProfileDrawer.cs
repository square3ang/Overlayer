...
public class ProfileDrawer : ModelDrawable<ProfileConfig> {
    ...
    public ProfileDrawer(OverlayerProfile profile) : base(profile.Config) {
    ...
    public override void Draw() {
        ...
        if (NeoDrawer.StaticInstance.DrawPath(Drawer.Icon_Pencil, Main.Lang.Get("NAME", "Name"), ref model.Name,
                Main.ProfilePath, "json")) ProfileManager.Rename(profile, model.Name);
        GUILayout.FlexibleSpace();
        GUILayout.EndHorizontal();
        if (NeoDrawer.StaticInstance.DrawSingleWithSlider(Drawer.Icon_Opacity, Main.Lang.Get("OPACITY", "Opacity"),
                ref model.Opacity, 0f, 1f, 300f)) profile.ApplyConfig();
        var old = GUI.color;
        GUILayout.BeginHorizontal();
        Type needCreateNewObject = null;
        if (Drawer.Button(Drawer.Icon_Font, GUILayout.Width(60))) needCreateNewObject = typeof(TextConfig);
        if (Drawer.Button(Drawer.Icon_Image, GUILayout.Width(40))) needCreateNewObject = typeof(ImageConfig);
        ...
        if (Drawer.Button(Drawer.Icon_Down, GUILayout.Width(60)))
            Task.Run(() => {
                ...
                foreach (var text in texts)
                    try {
                        if (Path.GetExtension(text) != ".json") continue;
                        ...
                        if (json is JArray arr) {
                            foreach (var token in arr) {
                                ...
                                ObjectConfig cfg = string.IsNullOrEmpty(type)
                                    ? new TextConfig()
                                    : type == "Text"
                                        ? TextConfigImporter.Import(token)
                                        : type == "Image"
                                            ? ImageConfigImporter.Import(token)
                                            : new TextConfig();
                ...
                Main.MainThreadDispatcher.Enqueue(() => {
                    foreach (var cfg in configsToAdd) profile.ObjectManager.Create(cfg);
                    dragSoltNeedInit = true;
                    profile.ObjectManager.Refresh();
                });
            });
        GUI.color = old;
        var showAs = Main.Settings.ShowTextNameAsDisplayText
        ...
        if (Drawer.Button(showAs)) Main.Settings.ShowTextNameAsDisplayText = !Main.Settings.ShowTextNameAsDisplayText;
        GUILayout.FlexibleSpace();
        GUILayout.EndHorizontal();
        ...
        for (var i = 0; i < profile.ObjectManager.Count; i++) {
            ...
            if (obj == null) {
                GUILayout.Label($"[{Main.Lang.Get("ERROR", "Error")}] " +
                                string.Format(
                                    Main.Lang.Get("ERROR_THIS_OBJECT_INDEX", "Unable to load object data at index {0}"),
                                    i));
                continue;
            }
            ...
            if (i != dragSoltDragging) {
                ...
                GUILayout.BeginHorizontal();
                if (Drawer.DrawOnlyBool(ref obj.Config.Active)) obj.gameObject.SetActive(obj.Config.Active);
                ...
                if (Drawer.Button(Drawer.Icon_X, GUILayout.Width(46))) {
                    ...
                    else if (Object.FindAnyObjectByType<DeletePopup>() == null) {
        ...
        if (dragSoltDragging >= 0) {
            if (Event.current.type == EventType.MouseUp) {
                ...
                GUILayout.BeginHorizontal();
                GUILayout.Space(20);
                var dmyActive = dobj.Config.Active;
        ...
        if (obj is OverlayerText text) {
            if (Main.Settings.ShowTextNameAsDisplayText) {
                if (text.Config.Active) {
                    var current = text.GetCurrentText();
                    var objName = current?.BreakRichTag();
    ...
    private Texture2D GetObjectIcon(OverlayerObject obj) {
        return obj is OverlayerText ? Drawer.Icon_Font : obj is OverlayerImage ? Drawer.Icon_Image : null;
    }
}