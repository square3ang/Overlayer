...
public class ImageConfigDrawer : ModelDrawable<ImageConfig> {
    ...
    public ImageConfigDrawer(OverlayerImage image) : base((ImageConfig)image.Config) {
        this.image = image;
    }
    ...
    public override void Draw() {
        ...
        var changed = false;
        changed |= Drawer.DrawExpr(Main.Lang.Get("POSITION", "Position"), "I_" + nameof(model.Position),
            ref model.Position, () => changed |= NeoDrawer.StaticInstance.DrawSize2(ref model.Position.Value, 0, 1),
            typeof(Vector2));
        changed |= Drawer.DrawExpr(Main.Lang.Get("SCALE", "Scale"), "I_" + nameof(model.Scale), ref model.Scale,
            () => changed |= NeoDrawer.StaticInstance.DrawSize2(ref model.Scale.Value, 0, 10), typeof(Vector2));
        if (IsAdvensedMode)
            changed |= Drawer.DrawExpr(Main.Lang.Get("PIVOT", "Pivot"), "I_" + nameof(model.Pivot), ref model.Pivot,
                () => changed |= NeoDrawer.StaticInstance.DrawSize2(ref model.Pivot.Value, 0, 1), typeof(Vector2));
        changed |= Drawer.DrawExpr(Main.Lang.Get("ROTATION", "Rotation"), "I_" + nameof(model.Rotation),
            ref model.Rotation,
            () => changed |= NeoDrawer.StaticInstance.DrawRotate3(ref model.Rotation.Value, -180, 180),
            typeof(Vector3));
        changed |= Drawer.DrawExpr(Drawer.Icon_Color, Main.Lang.Get("COLOR", "Color"), "I_" + nameof(model.Color),
            ref model.Color, () => {
                GUILayout.BeginHorizontal();
                ...
                if (paths.Length > 0)
                    Main.MainThreadDispatcher.Enqueue(() => {
                        foreach (var path in paths) {
                            var p = path.StartsWith(Main.Mod.Path)
            ...
            if (Drawer.Button(Drawer.Icon_OpenFolder, GUILayout.Width(40))) {
                ...
                Task.Run(() => {
                    ...
                    if (paths.Length > 0) {
                        ...
                        var finalPath = path;
                        Main.MainThreadDispatcher.Enqueue(() => {
                            if (index < currentModel.Images.Count) currentModel.Images[index] = finalPath;
                            image.ApplyImages();
                        });
                    }
                });
        ...
        Drawer.EndTab();
        changed |= Drawer.DrawCodeEditor(Drawer.Icon_Play, Main.Lang.Get("PLAYING_COMMAND", "Playing Command"),
            model.Name + "PlayingCommand", ref model.PlayingCommand);
        changed |= Drawer.DrawCodeEditor(Drawer.Icon_Pause, Main.Lang.Get("NOT_PLAYING_COMMAND", "Not Playing Command"),
            model.Name + "NotPlayingCommand", ref model.NotPlayingCommand);
        GUILayout.BeginHorizontal();
        ...
        if (Drawer.Button(Drawer.Icon_Up, GUILayout.Width(46)))
            Task.Run(() => {
                ...
                if (!string.IsNullOrWhiteSpace(target)) {
                    ...
                    node["Type"] = "Image";
                    if (Main.Settings.IncludeReferences) node["References"] = ImageConfigImporter.GetReferences(model);
