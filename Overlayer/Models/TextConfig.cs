...
public class TextConfig : ObjectConfig, ICopyable<TextConfig> {
    ...
    public string PlayingText =
        "<color=#{FOHex}>{Overloads}</color> <color=#{TEHex}>{OTE}</color> <color=#{VEHex}>{OVE}</color> <color=#{EPHex}>{OEP}</color> <color=#{PHex}>{OP}</color> <color=#{LPHex}>{OLP}</color> <color=#{VLHex}>{OVL}</color> <color=#{TLHex}>{OTL}</color> <color=#{FMHex}>{MissCount}</color>";
    ...
    public bool EnableFallbackFonts;
    ...
    TextConfig ICopyable<TextConfig>.Copy() {
    ...
    public override void Deserialize(JToken node) {
        ...
        OutlineColor.Deserialize(node[nameof(OutlineColor)],
            n => ModelUtils.ParseColorNode(n, defaults.OutlineColor.DefaultValue));
        ShadowColor.Deserialize(node[nameof(ShadowColor)],
            n => ModelUtils.ParseColorNode(n, defaults.ShadowColor.DefaultValue));
