...
public class FileAttempt : IModel, ICopyable<FileAttempt> {
    ...
    public void Deserialize(JToken node) {
        ...
        TileAttempts = [];
        if (node[nameof(TileAttempts)] is not JArray arr) return;
        foreach (var item in arr) {
            if (item is not JArray pair) continue;
            if (pair.Count != 2) continue;
            if (pair[0]?.Type != JTokenType.Integer || pair[1]?.Type != JTokenType.Integer) continue;
            TileAttempts.Add([
                pair[0]!.Value<int>(),
                pair[1]!.Value<int>()
            ]);
    ...
    public FileAttempt Copy() {
        ...
        foreach (var t in TileAttempts) copy.TileAttempts.Add([t[0], t[1]]);
        return copy;
    }

    private const string FileAttemptsFileName = "Overlayer_Attempts.json";

    public bool Save() {
        var path = GetPath();
        if (path == null) return false;

        var json = Serialize().ToString(Formatting.None);
    ...
    private static string GetPath() {
        if (scnGame.instance == null) return null;

        var level = scnGame.instance.levelPath;
