...
public class FastScrollView {
    ...
    public void DoGUI<T>(IEnumerable<T> items, Action<T> doGUIItem) {
        ...
        using (var sv = new GUILayout.ScrollViewScope(scrollPosition)) {
            ...
            if (scrollPosition != sv.scrollPosition) GUIUtility.keyboardControl = 0;
            ...
            using (new GUILayout.VerticalScope()) {
                if (needUpdateLayout) {
                    if (isRepaint) {
                        ...
                        foreach (var item in items) doGUIItem(item);
                    }
                }
                else {
                    ...
                    foreach (var item in itemRange) doGUIItem(item);
            ...
            if (needUpdateLayout && isRepaint) scopeWidth = GUILayoutUtility.GetLastRect().width;
        }
