...
public static partial class TypeUtility {
    ...
    private static readonly Dictionary<Type, List<MemberWrapper>> memberInfoTable = [];
    ...
    private static Dictionary<Type, List<PropertyInfo>> propertyInfoTable;

    private static List<PropertyInfo> GetPropertyInfoList(Type type) {
        if (propertyInfoTable == null) {
            ...
            propertyInfoTable = list.ToDictionary(tn => tn.type,
                tn => tn.names.Select(name => tn.type.GetProperty(name)).ToList());
        }
    ...
    private static readonly Dictionary<Type, List<FieldInfo>> fieldInfoTable = [];

    private static List<FieldInfo> GetFieldInfoList(Type type) {
        if (!fieldInfoTable.TryGetValue(type, out var fiList)) {
            fiList = type
                ...
                .Where(fi =>
                        ...
                        ? fi.GetCustomAttribute<NonSerializedAttribute>() == null
