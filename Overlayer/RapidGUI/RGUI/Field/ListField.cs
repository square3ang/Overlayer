...
public static partial class RGUI {
    private static readonly string[] ListPopupButtonNames = ["Add Element", "Delete Element"];

    public static T ListField<T>(T list, Func<T, int, string, object> customElementGUI = null,
        Func<T, object> customLabelRightFunc = null)
        where T : IList {
    ...
    public static T ListField<T>(T list, string label, Func<T, int, string, object> customElementGUI = null,
        Func<T, object> customLabelRightFunc = null)
        where T : IList {
        var labelRightFunc = ListLabelRightFunc;
        ...
        return (T)DoField(list, typeof(T), label, styleNone,
            (v, t) => ListField(v, t, customElementGUI),
            labelRightFunc,
            null
        );
    }

    public static T ListLabelRightFunc<T>(T v) where T : IList {
        return (T)ListLabelRightFunc(v, typeof(T));
    }

    private static object ListLabelRightFunc(object v, Type type) {
        var list = v as IList;
    ...
    private static object ListField(object v, Type type) {
        return ListField<object>(v, type, null);
    }

    private static object ListField<T>(object v, Type type, Func<T, int, string, object> customElementGUI) {
        var list = v as IList;
    ...
    private static IList AddElementAtLast(IList list, Type type, Type elemType) {
    ...
    private static IList DeleteElementAtLast(IList target, Type elemType) {
        return DeleteElement(target, elemType, target.Count - 1);
    }

    private static IList AddElement(IList list, Type elemType, object baseElem, int index) {
    ...
    private static IList DeleteElement(IList list, Type elemType, int index) {
        if (list is Array array) {
            var newArray = Array.CreateInstance(elemType, array.Length - 1);
            Array.Copy(array, newArray, index);
    ...
    private static object CreateNewElement(object baseElem, Type elemType) {
        object ret = null;
        ...
        if (baseElem != null) {
            ...
            else if (elemType.GetConstructor([elemType]) != null) ret = Activator.CreateInstance(elemType, baseElem);
        }
