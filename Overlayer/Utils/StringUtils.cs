...
public static class StringUtils {
    ...
    public static readonly Regex RichTagBreaker =
        new(@"<(color|material|quad|size)=(.|\n)*?>|<\/(color|material|quad|size)>|<(b|i)>|<\/(b|i)>",
            RegexOptions.Compiled | RegexOptions.Multiline);

    public static readonly Regex RichTagBreakerWithoutSize =
        new(@"<(color|material|quad)=(.|\n)*?>|<\/(color|material|quad)>|<(b|i)>|<\/(b|i)>",
            RegexOptions.Compiled | RegexOptions.Multiline);

    public static readonly Regex RichTagColorFixer = new(@"<color=([""']?#?[0-9A-Fa-f]+[""']?)>(.*?)</color>",
        RegexOptions.Compiled | RegexOptions.Singleline);

    public static string BreakRichTag(this string s) {
        return RichTagBreaker.Replace(s, string.Empty);
    }

    public static string BreakRichTagWithoutSize(this string s) {
        return RichTagBreakerWithoutSize.Replace(s, string.Empty);
    }

    public static string FuckingAdofaiMapRichTagFixer(this string s) {
        return RichTagColorFixer.Replace(s, m => {
    ...
    public static double CalculateDifference(this string a, string b) {
        return string.IsNullOrEmpty(a) || string.IsNullOrEmpty(b)
            ...
            : a == b
                ? 1.0
                : 1.0 - a.GetDifference(b) / Math.Max(a.Length, b.Length);
    }
    ...
    public static string GetBetween(this string str, string start, string end) {
        var sIdx = str.IndexOf(start);
        var eIdx = str.LastIndexOf(end);
    ...
    public static int GetDifference(this string a, string b) {
        var aCount = a.Length;
        var bCount = b.Length;
        var d = new int[aCount + 1, bCount + 1];
        if (aCount == 0) return bCount;

        if (bCount == 0) return aCount;

        for (var i = 0; i <= aCount; d[i, 0] = i++) ;

        for (var i = 0; i <= bCount; d[0, i] = i++) ;
    ...
    public static unsafe string InvertAlternately(this string s) {
        ...
        var isLower = s[0].IsLower();
        fixed (char* ptr = s) {
            var c = ptr;
            char v;
            while ((v = *c) != '\0') {
                if (v.IsAlphabet()) *c = (isLower = !isLower) ? v.ToLower() : v.ToUpper();
    ...
    public static bool IsAlphabet(this char c) {
        return c.IsUpper() || c.IsLower();
    }

    public static bool IsLower(this char c) {
        return c is >= (char)Lower and <= (char)LowerLast;
    }

    public static bool IsUpper(this char c) {
        return c is >= (char)Upper and <= (char)UpperLast;
    }
