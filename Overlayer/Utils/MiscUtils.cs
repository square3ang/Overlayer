    ...
    public static class MiscUtils {
        ...
        public static Color ShiftValue(Color color, float amount) {
            Color.RGBToHSV(color, out _, out _, out var val);
            val += amount;
        ...
        public static Type TypeByName(string typeName) {
            ...
            loadedTypes ??= loadedAsss.Select(ass => ExecuteSafe(ass.GetTypes, out _)).Where(t => t != null)
                .SelectMany(ts => ts).ToArray();
        ...
        public static Vector2 AlignmentToPivot(TextAlignmentOptions alignment) {
            var x = ((int)alignment & (1 << 0)) != 0 ? 0f :
                ((int)alignment & (1 << 2)) != 0 ? 1f : 0.5f;

            var y = ((int)alignment & (1 << 8)) != 0 ? 1f :
        ...
        public static bool IsHovering() {
            if (Event.current.type == EventType.Repaint) _lastRect = GUILayoutUtility.GetLastRect();
        ...
        private static float NextFloat(string s, ref int idx) {
            ...
            var v = float.Parse(s.Substring(start, idx - start), CultureInfo.InvariantCulture);
            idx++;
        ...
        public static Vector2 ParseVec2(string s) {
            ...
            return new Vector2(
                NextFloat(s, ref idx),
                float.Parse(s.Substring(idx), CultureInfo.InvariantCulture)
            );
        }
        ...
        public static Vector3 ParseVec3(string s) {
            ...
            var x = NextFloat(s, ref idx);
            var y = NextFloat(s, ref idx);
            var z = float.Parse(s.Substring(idx), CultureInfo.InvariantCulture);
        ...
        public static Color ParseColor(string s) {
            ...
            var r = NextFloat(s, ref idx);
            var g = NextFloat(s, ref idx);
            var b = NextFloat(s, ref idx);
            var a = float.Parse(s.Substring(idx), CultureInfo.InvariantCulture);

            return new Color(r, g, b, a);
        }

        public static TextAlignmentOptions ParseAlign(string s) {
            return (TextAlignmentOptions)int.Parse(s);
        }
    }