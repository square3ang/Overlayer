...
public static class AutoUpdater {
    ...
    public static readonly string OverlayerGithubApiLink =
        "https://api.github.com/repos/modlist-org/Overlayer/releases";

    public static bool IsRateLimited { get; private set; }

    public static void Reload(ModEntry modEntry) {
        var entryType = typeof(ModEntry);
        var canReloadProp = entryType.GetProperty("CanReload",
            BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public);
        var setMethod = canReloadProp?.GetSetMethod(true);
        var reloadMethod = entryType.GetMethod("Reload", BindingFlags.Instance | BindingFlags.NonPublic);
        setMethod?.Invoke(modEntry, [true]);
        reloadMethod?.Invoke(modEntry, null);
        setMethod?.Invoke(modEntry, [false]);
    }

    public static async Task InitAndUpdate(ModEntry modEntry, bool update = false, bool allowBeta = false,
        Action ok = null, Action<string> err = null, bool latestIsError = false) {
        await InitUpdate(modEntry.Version, async () => {
            if (update) await CheckAndPrepareUpdate(modEntry, allowBeta, ok, err, latestIsError);
        }, err);
    }
        ...
        try {
            ...
            if (releases
                ...
                if (CurrentVersionType == VersionType.Beta) {
                    ...
                    else if (currentVersion < BetaVersion) CurrentVersionType = VersionType.OldBeta;
                }
            }
            else {
                BetaUrl = null;
            ...
            err?.Invoke(Main.Lang.Get("UPDATER_VERSION_CHECK_ERROR", "Version parse or asset fetch failed") + ": " +
                        ex.Message);
        }
    }

    public static async Task CheckAndPrepareUpdate(ModEntry modEntry, bool allowBeta = false, Action ok = null,
        Action<string> err = null, bool latestPassIsError = false) {
        if (IsUpdating) {
        ...
        if (IsLatest && (!allowBeta || CurrentVersionType == VersionType.OldBeta)) {
            var msg = Main.Lang.Get("UPDATER_ALREADY_LATEST", "Already the latest version");
        ...
        var url = allowBeta && !string.IsNullOrEmpty(BetaUrl) && BetaVersion > LatestVersion ? BetaUrl : LatestUrl;
        ...
        var tempDir = Path.Combine(modEntry.Path, "updatetemp");
        var zipPath = Path.Combine(tempDir, "Overlayer.zip");
        ...
        try {
            ...
            if (File.Exists(infoPath))
                try {
                    ...
                    var extractedVersion = infoJson["Version"]?.ToString();
                    if (!string.IsNullOrEmpty(extractedVersion) && extractedVersion == modEntry.Version.ToString()) {
                        err?.Invoke(Main.Lang.Get("UPDATER_SAME_VERSION_DETECTED",
                            "Update package version matches the current version") + ": " + extractedVersion);
                        return;
                    }
                }
                catch (Exception e) {
                    err?.Invoke(Main.Lang.Get("UPDATER_INFO_JSON_PARSE_FAILED", "Failed to parse info.json") + ": " +
                                e.Message);
                    return;
                }
            else
    ...
    public static Version UpdateBeforeLoad(ModEntry modEntry) {
        ...
        try {
            var infoJson = JObject.Parse(File.ReadAllText(infoPath));
            var versionStr = infoJson["Version"]?.ToString();

            if (string.IsNullOrEmpty(versionStr)) {
                var errPkg = "Failed to get version from info.json.";
            ...
            var errParse = "Failed to parse info.json: " + e.Message;
        ...
        try {
            foreach (var file in Directory.GetFiles(tempDir, "*", SearchOption.AllDirectories)) {
                ...
                var destPath = Path.Combine(modEntry.Path, relativePath);
