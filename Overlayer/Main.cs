...
public static class Main {
    [Tags.Attributes.Tag(NotPlaying = true)]
    public static string Developer => Lang.Get("MISC_DEVELOPER",
        "Square3ang & Kkitut. Display everything as you wish. Thank you for being with Overlayer.");
    ...
    [Tags.Attributes.Tag("imBBBT", NotPlaying = true)]
    ...
    [Tags.Attributes.Tag(NotPlaying = true)]
    public static Version ModVersion => Mod.Version;

    public static bool IsShowGUI { get; private set; }
    private static UpdatePopup popup;

    public static string tooltip = "";
    public static Texture2D tooltipImage;
    ...
    private static bool _egEnabled;
    ...
    internal static bool EgEnabled {
        ...
        set {
            if (_egEnabled != value) {
                if (value) {
                    if (OllyResources.LoadAll(Mod)) {
                        Eg = new GameObject().AddComponent<Olly.Olly>();
                        Object.DontDestroyOnLoad(Eg);
                        Eg.Init();
                        _egEnabled = value;
                    }
                }
                else {
                    Object.Destroy(Eg.gameObject);
                    Eg.Release();
                    Eg = null;
                    OllyResources.UnloadAll();
                    _egEnabled = value;
                }
            }
        }
    ...
    public static void Load(ModEntry modEntry) {
        ...
        var needReload = AutoUpdater.UpdateBeforeLoad(modEntry);
        if (needReload != null) {
            var versionField = typeof(ModEntry).GetField("Version", BindingFlags.Instance | BindingFlags.Public);
        ...
        MiscUtils.SetAttr(TMP_Settings.instance, "m_warningsDisabled", true);
    }
    ...
    public static bool OnToggle(ModEntry modEntry, bool toggle) {
        if (toggle) {
            ...
            if (!Settings.DisableLogo) LogoInit(modEntry.Path);
            StaticCoroutine.Run(null);
            StaticCoroutine.Run(LoadCoroutine(modEntry));
            Scripting.Initalize();
            if (Settings.FileAttempt) FileAttempt = new FileAttempt();
            ...
            if (updateOnce) {
                ...
                _ = AutoUpdater.InitAndUpdate(
                    ...
                    () => UpdateInfo = Lang.Get("UPDATE_SUCESS", "Update Success!") + " " +
                                       Lang.Get("UPDATE_NEED_TO_RESTART", "Need to Restart!"),
                    err => {
    ...
    public static void OnGUI(ModEntry modEntry) {
        if (!AutoUpdater.IsLatest) {
            GUILayout.Label(
                $"<size=50><color=red>{Lang.Get("OUTDATED_DESCRIPTION", "Outdated Version Detected!")}</color></size>");
        ...
        GUILayout.Space(30);
        GUILayout.BeginHorizontal();
        if (Drawer.Button(Drawer.Icon_Discord, " Discord")) Application.OpenURL("https://discord.modlist.org/");
        ...
        if (AutoUpdater.CurrentVersionType != AutoUpdater.VersionType.Unknown) {
            if (AutoUpdater.LatestVersion != null) {
                ...
                if (AutoUpdater.CurrentVersionType == AutoUpdater.VersionType.Stable) GUILayout.Label(" <<");
                GUILayout.FlexibleSpace();
                GUILayout.EndHorizontal();
            }
            ...
            if (AutoUpdater.BetaVersion != null) {
                ...
                if (AutoUpdater.CurrentVersionType == AutoUpdater.VersionType.Beta) GUILayout.Label(" <<");
                GUILayout.FlexibleSpace();
                GUILayout.EndHorizontal();
            }

            if (AutoUpdater.CurrentVersionType == AutoUpdater.VersionType.UnknownBeta)
                GUILayout.Label($"You are using an <color=#{Effect.Rainbow(12)}>SPECIAL BETA!</color>");
        }
        else if (AutoUpdater.IsRateLimited) {
            GUILayout.Label(
                $"<color=yellow>{Lang.Get("UPDATE_RATE_LIMITED", "Update or Version check Rate Limited!")}</color>");
        }
    ...
    public static void LogoInit(string path) {
        var logopath = Path.Combine(path, "ov3_logo.png");
        if (File.Exists(logopath)) {
            var fileData = File.ReadAllBytes(logopath);
    ...
    public static class MainThreadDispatcher {
        ...
        public static void Update() {
            while (queue.TryDequeue(out var action)) action.Invoke();
        }
    }
}