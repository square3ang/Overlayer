...
public class OverlayerText : OverlayerObject, IPointerDownHandler, IPointerUpHandler, IDragHandler,
    ...
    private static int pointingCount;
    ...
    static OverlayerText() {
        sr_msdf = (Shader)typeof(ShaderUtilities).GetProperty("ShaderRef_MobileSDF", (BindingFlags)15420)
            .GetValue(null);
    }
    ...
    public void Init(OverlayerProfile profile, TextConfig config) {
        ...
        Parent = profile;
        TextConfig = config;
        if (string.IsNullOrEmpty(config.Name)) config.Name = $"Text {Parent.ObjectManager.Count + 1}";
    ...
    public void Update() {
        if (!Initialized || Text is null) return;

        Text.text = Main.IsPlaying ? PlayingReplacer?.Replace() ?? "" : NotPlayingReplacer?.Replace() ?? "";

        if (TextConfig.LineSpacing.GetExprValue(float.Parse, out var ls)) Text.lineSpacing = ls;
        if (TextConfig.LineSpacingAdj.GetExprValue(float.Parse, out var lsa)) Text.lineSpacingAdjustment = lsa;
        if (TextConfig.TextColor.GetExprValue(MiscUtils.ParseGColor, out var color)) Text.colorGradient = color;
        if (TextConfig.Pivot.GetExprValue(MiscUtils.ParseVec2, out var pivot)) Text.rectTransform.pivot = pivot;
        if (TextConfig.Scale.GetExprValue(MiscUtils.ParseVec2, out var scale)) Text.rectTransform.localScale = scale;
        ...
        if (TextConfig.Rotation.GetExprValue(MiscUtils.ParseVec3, out var rot)) Text.rectTransform.eulerAngles = rot;
        if (TextConfig.FontSize.GetExprValue(float.Parse, out var fs)) Text.fontSize = fs;
        ...
        if (_instancedMaterials != null) UpdateMaterialExpressions();
    ...
    public override void ApplyConfig() {
        ...
        if (TextConfig.LineSpacing.GetNormalValue(out var ls)) Text.lineSpacing = ls;
        if (TextConfig.LineSpacingAdj.GetNormalValue(out var lsa)) Text.lineSpacingAdjustment = lsa;
        if (TextConfig.TextColor.GetNormalValue(out var color)) Text.colorGradient = color;
        if (TextConfig.Pivot.GetNormalValue(out var pivot)) Text.rectTransform.pivot = pivot;
        if (TextConfig.Scale.GetNormalValue(out var scale)) Text.rectTransform.localScale = scale;
        if (TextConfig.Position.GetNormalValue(out var pos))
            Text.rectTransform.anchoredPosition = (pos - new Vector2(0.5f, 0.5f)) * new Vector2(1920, 1080);
        if (TextConfig.Rotation.GetNormalValue(out var rot)) Text.rectTransform.eulerAngles = rot;
        if (TextConfig.FontSize.GetNormalValue(out var fs)) Text.fontSize = fs;
    ...
    public void OnPointerDown(PointerEventData e) {
        if (!CanDrag || isAlreadyDragging) return;

        isDragging = true;
        isAlreadyDragging = true;

        var parentRect = Text.rectTransform.parent as RectTransform;
    ...
    public void OnDrag(PointerEventData e) {
        ...
        var offset = currentPointerLocal - initialPointerLocal;
    ...
    public void OnPointerExit(PointerEventData e) {
        ...
        if (pointingCount <= 0) {
            pointingCount = 0;
            if (!isAlreadyDragging) OverlayerProfile.DragObj.SetActive(false);
        }
    ...
    private void SetFont() {
        if (!FontManager.TryGetFont(TextConfig.Font, out var font)) return;

        var targetFont = font.fontTMP;
    ...
    private void UpdateMaterials() {
        ...
        foreach (var mat in _instancedMaterials) {
            ...
            if (TextConfig.OutlineColor.GetNormalValue(out var oc)) mat.SetColor(ShaderUtilities.ID_OutlineColor, oc);
            if (TextConfig.OutlineWidth.GetNormalValue(out var ow)) mat.SetFloat(ShaderUtilities.ID_OutlineWidth, ow);
            if (TextConfig.ShadowColor.GetNormalValue(out var sc)) mat.SetColor(ShaderUtilities.ID_UnderlayColor, sc);
    ...
    private void RefreshMaterials(Material[] shared) {
        ...
        for (var i = 0; i < shared.Length; i++) {
            if (shared[i] is null) continue;
            _instancedMaterials[i] = new Material(shared[i]);
            if (sr_msdf) _instancedMaterials[i].shader = sr_msdf;
