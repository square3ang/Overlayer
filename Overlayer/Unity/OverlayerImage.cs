...
public class OverlayerImage : OverlayerObject, IPointerDownHandler, IPointerUpHandler, IDragHandler,
    ...
    public void Update() {
        if (!Initialized || _mainImage is null || Images.Count == 0) return;

        var rawCommand = Main.IsPlaying ? PlayingReplacer?.Replace() ?? "" : NotPlayingReplacer?.Replace() ?? "";

        int idx;
        _mainImage.sprite = int.TryParse(rawCommand, out idx) && idx >= 0 && idx < Images.Count
            ? Images[idx]
            : ImageManager.DefaultSprite;

        if (ImageConfig.Color.GetExprValue(MiscUtils.ParseColor, out var color)) _mainImage.color = color;
        if (ImageConfig.Pivot.GetExprValue(MiscUtils.ParseVec2, out var pivot)) _mainImage.rectTransform.pivot = pivot;
        ...
        if (isDragging && OverlayerProfile.DragObj is not null && OverlayerProfile.DragImage is not null) {
            ...
            OverlayerProfile.DragImage.rectTransform.sizeDelta =
                new Vector2(_mainImage.preferredWidth, _mainImage.preferredHeight) * scaleforDrag;
        }
    }
    ...
    public override void ApplyConfig() {
        ...
        if (ImageConfig.Color.GetNormalValue(out var color)) _mainImage.color = color;
        if (ImageConfig.Pivot.GetNormalValue(out var pivot)) _mainImage.rectTransform.pivot = pivot;
        ...
        if (ImageConfig.Rotation.GetNormalValue(out var rot)) _mainImage.rectTransform.rotation = Quaternion.Euler(rot);
    ...
    public void ApplyImages() {
        ...
        for (var i = 0; i < ImageConfig.Images.Count; i++) {
            var imagePath = ImageConfig.Images[i];
    ...
    public void OnPointerDown(PointerEventData e) {
        ...
        var parentRect = _mainImage.rectTransform.parent as RectTransform;
        RectTransformUtility.ScreenPointToLocalPointInRectangle(parentRect, e.position, e.pressEventCamera,
    ...
    public void OnPointerExit(PointerEventData e) {
        ...
        if (pointingCount <= 0) {
            pointingCount = 0;
            if (!isAlreadyDragging && OverlayerProfile.DragObj != null) OverlayerProfile.DragObj.SetActive(false);
        }
