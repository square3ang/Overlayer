...
public static class FontPatch {
    ...
    public static class FontChanger {
        public static bool Prefix(ref FontData __result) {
            if (!FontManager.Initialized) return true;

            if (!Main.Settings.ChangeFont) return true;

            if (!FontManager.TryGetFont(Main.Settings.AdofaiFont.name, out var font)) return true;

            if (!(font.font?.dynamic ?? false)) return true;
    ...
    public static class FontAttacher {
        ...
        private static IEnumerator UpdateFontCo() {
            if (!Main.Settings.ChangeFont) yield break;

            List<GameObject> list = [];
            try {
                Main.ActiveScene.GetRootGameObjects(list);
            }
            catch {
                yield break;
            }
            ...
            foreach (var i in list) {
                ...
                if (i.GetComponentInChildren(typeof(scrEnableIfBeta))) continue;

                foreach (var j in i.GetComponentsInChildren<Text>(false)) j.SetLocalizedFont();
