...
public class OverlayerTag {
    ...
    private static MethodInfo WrapProcessor(MemberInfo fieldPropMethod, object target, ValueProcessing flags,
        ...
        var t = mod.DefineType($"ValueProcessor_{fieldPropMethod?.Name}${uniqueNum++}", TypeAttributes.Public);
        var m = t.DefineMethod("Getter", MethodAttributes.Public | MethodAttributes.Static);
        var targetField = t.DefineField("target", target?.GetType() ?? typeof(object),
        ...
        if (fieldPropMethod is FieldInfo field) {
            if (!field.IsStatic && target == null)
                throw new InvalidOperationException(
                    $"Field '{field.Name}' Cannot Get Instance Member Without Target!!");

            if (!field.IsStatic && target != null) il.Emit(OpCodes.Ldsfld, targetField);
        ...
        if (fieldPropMethod is MethodInfo method) {
            ...
            if (!method.IsStatic && target == null)
                throw new InvalidOperationException(
                    $"Method '{method.Name}' Cannot Call Instance Member Without Target!!");

            if (!method.IsStatic && target != null) il.Emit(OpCodes.Ldsfld, targetField);
        ...
        if ((flags & ValueProcessing.RoundNumber) != 0) {
            if (rt != typeof(double)) il.Emit(OpCodes.Conv_R8);

            il.Emit(OpCodes.Ldarg, parameters.Count);
            il.Emit(OpCodes.Call, round);
            if (rt != typeof(double)) il.Convert(rt);
    ...
    public static void Initialize() {
        ...
        uniqueNum = 0;
        ass = AssemblyBuilder.DefineDynamicAssembly(new AssemblyName("Overlayer.Tags.FieldTagWrappers"),
    ...
    public static object RuntimeAccess(object obj, string accessor = "") {
        ...
        for (var i = 0; i < accessors.Length; i++) {
            ...
            if (accessors[i].Equals("ListMembers", StringComparison.OrdinalIgnoreCase)) {
                ...
                for (var j = 0; j < members.Length; j++) {
                    ...
                    if (m is FieldInfo field)
                        sb.AppendLine(
                            $"{(field.IsPublic ? "Public" : "Private")}{(field.IsStatic ? " Static" : "")} Field {field.FieldType} '{field.Name}'");
                    else if (m is PropertyInfo prop && prop.GetGetMethod(true) is MethodInfo getter)
                        sb.AppendLine(
                            $"{(getter.IsPublic ? "Public" : "Private")}{(getter.IsStatic ? " Static" : "")} Property {prop.PropertyType} '{prop.Name}'");
                }
            ...
            var member = foundMembers.Where(m => m.MemberType is MemberTypes.Field or MemberTypes.Property)
                .FirstOrDefault();
            result = member is FieldInfo f
                ? f.GetValue(result)
                : member is PropertyInfo p && p.GetGetMethod(true) != null
                    ? p.GetValue(result)
                    : null;
        ...
        accessorCache[$"{objType}_Accessor_{accessor}"] =
            (Func<object, object>)CreateMemberAccessor(objType, accessor, staticAccess)
    ...
    public static DynamicMethod CreateMemberAccessor(Type type, string accessor, bool staticAccess) {
        var name = $"{type}_Accessor_{accessor}" + (staticAccess ? "_Static" : "");
        if (accessorCacheDM.TryGetValue(name, out var dm)) return dm;

        var accessors = accessor.Split(['.'], StringSplitOptions.RemoveEmptyEntries);
        ...
        for (var i = 0; i < accessors.Length; i++) {
            ...
            result = ignoreCase
                ? t?.GetMembers((BindingFlags)15420)
                    .Where(m => m.Name.Equals(accessors[i], StringComparison.OrdinalIgnoreCase)).FirstOrDefault()
                : t?.GetMember(accessors[i], MemberTypes.Field | MemberTypes.Property, (BindingFlags)15420)
                    .FirstOrDefault();
        ...
        var last = toEmitMembers.Last();
        var rt = last is FieldInfo ff ? ff.FieldType : last is PropertyInfo pp ? pp.PropertyType : typeof(object);
        accessorCacheDM[name] =
            dm = new DynamicMethod(name, typeof(object), [typeof(object)], typeof(OverlayerTag), true);
        var il = dm.GetILGenerator();
        if (!staticAccess) il.Emit(OpCodes.Ldarg_0);

        foreach (var m in toEmitMembers)
            if (m is FieldInfo f) {
                if (f.IsStatic)
        ...
        if (typeof(object) != rt) il.Emit(OpCodes.Box, rt);
    ...
    private static readonly MethodInfo round;
    private static readonly MethodInfo trim;
    private static MethodInfo toString;
    private static readonly MethodInfo runtimeAccessor;
    private static AssemblyBuilder ass;
    private static ModuleBuilder mod;
    private static readonly Dictionary<string, Func<object, object>> accessorCache = [];
    private static readonly Dictionary<string, DynamicMethod> accessorCacheDM = [];

    static OverlayerTag() {
        runtimeAccessor = typeof(OverlayerTag).GetMethod(nameof(RuntimeAccess), (BindingFlags)15420, null, [
