...
public static class Level {
    ...
    [Tag] public static string LevelNameText;
    [Tag] public static string LevelNameTextRaw;
    ...
    public static void Init() {
        ...
        if (levelData == null) {
            ...
            _defaultTextColorAlpha = LevelNameTextColor();
            _defaultTextShadowColorAlpha = LevelNameTextShadowColor();
        }
        else {
            _defaultTextColor = ADOFAI.LevelData.defaultTextColor.ToHex();
            _defaultTextShadowColor = ADOFAI.LevelData.defaultTextShadowColor.ToHex();
    ...
    public static string Title(int maxLength = -1, string afterTrimStr = Extensions.DefaultTrimStr) {
        return string.IsNullOrEmpty(_title) ? "" : _title.Trim(maxLength, afterTrimStr);
    }

    [Tag]
    public static string Author(int maxLength = -1, string afterTrimStr = Extensions.DefaultTrimStr) {
        return string.IsNullOrEmpty(_author) ? "" : _author.Trim(maxLength, afterTrimStr);
    }

    [Tag]
    public static string Artist(int maxLength = -1, string afterTrimStr = Extensions.DefaultTrimStr) {
        return string.IsNullOrEmpty(_artist) ? "" : _artist.Trim(maxLength, afterTrimStr);
    }

    [Tag]
    public static string TitleRaw(int maxLength = -1, string afterTrimStr = Extensions.DefaultTrimStr) {
        return string.IsNullOrEmpty(_titleRaw) ? "" : _titleRaw.Trim(maxLength, afterTrimStr);
    }

    [Tag]
    public static string AuthorRaw(int maxLength = -1, string afterTrimStr = Extensions.DefaultTrimStr) {
        return string.IsNullOrEmpty(_authorRaw) ? "" : _authorRaw.Trim(maxLength, afterTrimStr);
    }

    [Tag]
    public static string ArtistRaw(int maxLength = -1, string afterTrimStr = Extensions.DefaultTrimStr) {
        return string.IsNullOrEmpty(_artistRaw) ? "" : _artistRaw.Trim(maxLength, afterTrimStr);
    }

    [Tag]
    public static string DefaultTextColor(bool noAlpha = false) {
        return noAlpha ? _defaultTextColor : _defaultTextColorAlpha;
    }

    [Tag]
    public static string DefaultTextShadowColor(bool noAlpha = false) {
        return noAlpha ? _defaultTextShadowColor : _defaultTextShadowColorAlpha;
    }

    [Tag]
    public static string LevelNameTextColor(bool noAlpha = false) {
        return scrVfx.instance.currentColourScheme.colourText.ToHex(!noAlpha);
    }

    [Tag]
    public static string LevelNameTextShadowColor(bool noAlpha = false) {
        return scrVfx.instance.currentColourScheme.colourTextShadow.ToHex(!noAlpha);
    }
}