...
public class GUIController {
    ...
    private bool hasOnceCalled;

    public void Draw() {
        if (skipFrames > 0) {
            skipFrames--;
            if (onSkipCallbacks.Count > 0) onSkipCallbacks.Pop()?.Invoke();
        ...
        GUILayout.BeginHorizontal();
        {
            ...
            if (isRedoAvailable) {
                ...
                if (Drawer.Button(draw.Name + "▶ " + Main.Lang.Get("FORWARD", "Forward"))) Push(draw);
            }
        }
        ...
        if (isUndoAvailable) {
            GUILayout.BeginHorizontal();
            if (Drawer.Button("◀ " + Main.Lang.Get("BACK", "Back") + drawables[depth - 1].Name)) Pop();
