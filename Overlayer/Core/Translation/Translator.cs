...
public class Translator {
    ...
    public bool IsDefault => (IsFail && FailState != TranslationFailState.SomeFailure) || IsLoading ||
        ...
        foreach (var file in files) {
            ...
            foreach (var property in jsonObject.Properties()) {
                ...
                if (!block.TryGetValue(KTLKey, out var ktToken) || ktToken.ToString() != ExpectedKTLValue) {
                    Log(
                        $"{LOG_PREFIX}Invalid or missing {DEFAULT_KTL_KEY} in file: {file}, block: {property.Name}, passing");
                    continue;
                }
                ...
                if (stringDict.Count > 0) translations[property.Name] = stringDict;
                if (arrayDict.Count > 0) translationsArr[property.Name] = arrayDict;
            }
        }
