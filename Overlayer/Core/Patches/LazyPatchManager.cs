...
internal static class LazyPatchManager {
    ...
    public static void Load(Assembly ass) {
        foreach (var type in ass.GetTypes()) {
            ...
            if (lpas.Any()) {
                if (!Patches.TryGetValue(type, out var list)) list = Patches[type] = [];

                foreach (var patch in lpas) list.Add(new LazyPatch(Harmony, type, patch));
            }
        }
    }
    ...
    public static void PatchInternal() {
        PatchAll(LazyPatch.InternalTrigger);
    }
    ...
    public static List<LazyPatch> PatchAll(string trigger = null) {
        ...
        if (trigger != null) {
            if (PatchedTriggers.Add(trigger))
                foreach (var patch in patches = Patches.Values.SelectMany(list => list)
                             .Where(p => p.attr.Triggers.Contains(trigger)).ToList())
                    patch.Patch();
        }
        else {
            foreach (var patchType in Patches.Keys) patches.AddRange(Patch(patchType));
        }
    ...
    public static List<LazyPatch> UnpatchAll(string trigger = null) {
        ...
        if (trigger != null) {
            ...
            foreach (var patchType in Patches.Keys) patches.AddRange(Unpatch(patchType));
        }
    ...
    public static List<LazyPatch> GetLockedPatches() {
        return Patches.Values.SelectMany(l => l).Where(lp => lp.Locked).ToList();
    }

    public static List<LazyPatch> GetUnlockedPatches() {
        return Patches.Values.SelectMany(l => l).Where(lp => !lp.Locked).ToList();
    }

    public static List<LazyPatch> GetPatchedPatches() {
        return Patches.Values.SelectMany(l => l).Where(lp => lp.Patched).ToList();
    }

    public static List<LazyPatch> GetUnpatchedPatches() {
        return Patches.Values.SelectMany(l => l).Where(lp => !lp.Patched).ToList();
    }
