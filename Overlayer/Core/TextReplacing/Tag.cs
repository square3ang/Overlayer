...
public class Tag {
    ...
    public Tag SetGetter(MethodInfo method, object target = null) {
        ...
        GetterOriginalDelegate =
            method.CreateDelegate(Expression.GetFuncType(parameters.Select(p => p.ParameterType)
                .Append(method.ReturnType).ToArray()));
        ReturnType = method.ReturnType;
        Getter = WrapMethodToGetter(out var gDel);
        GetterDelegate = gDel;
    ...
    private static AssemblyBuilder TagWrapperAssembly;
    private static ModuleBuilder TagWrapperModule;
    private static int uniqueId;
    private static bool wrapperInitialized;

    private MethodInfo WrapMethodToGetter(out Delegate del) {
        del = GetterDelegate;
        if (Getter != null) return Getter;

        string methodName, targetFieldName;
        var t = TagWrapperModule.DefineType($"{Name}_WrapperType${uniqueId++}", TypeAttributes.Public);
        var parameters = GetterOriginal.GetParameters();
        var parameterTypes = Enumerable.Repeat(typeof(string), parameters.Length).ToArray();
        var m = t.DefineMethod(methodName = $"{Name}_WrapperMethod", MethodAttributes.Public | MethodAttributes.Static,
            typeof(string), parameterTypes);
        var targetField = t.DefineField(targetFieldName = "Target", GetterOriginal.DeclaringType,
            FieldAttributes.Public | FieldAttributes.Static);
        var il = m.GetILGenerator();
        if (!GetterOriginal.IsStatic) il.Emit(OpCodes.Ldsfld, targetField);

        for (var i = 0; i < ArgumentConverter.Length; i++) {
        ...
        if (ReturnConverter != null) il.Emit(OpCodes.Call, ReturnConverter);
    ...
    private static MethodInfo WrapDelegateDirect(string name, Delegate del) {
        var delType = del.GetType();
        var invoke = delType.GetMethod("Invoke");
        var method = del.Method;
        var type = TagWrapperModule.DefineType($"{name}_Delegate_WrapperType${uniqueId++}", TypeAttributes.Public);
        var parameters = method.GetParameters();
        var paramTypes = parameters.Select(p => p.ParameterType).ToArray();
        var methodB = type.DefineMethod($"{name}_Delegate_WrapperMethod",
            MethodAttributes.Public | MethodAttributes.Static, invoke.ReturnType, paramTypes);
        var delField = type.DefineField("del", delType, FieldAttributes.Public | FieldAttributes.Static);
    ...
    public static void InitializeWrapperAssembly() {
        ...
        TagWrapperAssembly = AssemblyBuilder.DefineDynamicAssembly(new AssemblyName("Overlayer.TagWrapper"),
