...
public static class Lexer {
    public static List<Token> Lex(string source, LexConfig config = null) {
        ...
        if (invalid >= 0) {
            ...
            for (var i = invalid; i < lex.Count; i++) values.Append(lex[i].value);
    ...
    private static IEnumerable<Token> Lex_(string source, LexConfig config = null) {
        ...
        for (var i = 0; i < source.Length; i++) {
            ...
            if (c == config.TagStart) {
                ...
                if (sb.Length > 0) yield return new Token(TokenType.Identifier, sb.ToString());
