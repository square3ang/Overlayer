...
public class Replacer {
    ...
    public Replacer(string source, List<Tag> tags = null) : this(tags) {
        Source = source;
    }
    ...
    public void Dispose() {
        interpretable?.Dispose();
        interpretable = null;
        foreach (var tag in References) tag.ReferencedCount--;
        References.Clear();
        compiledMethod = null;
        compiled = false;
    }
    ...
    public static readonly MethodInfo StrBuilder_Append =
        typeof(StringBuilder).GetMethod("Append", new[] { typeof(string) });

    public static readonly MethodInfo
        StrBuilder_ToString = typeof(StringBuilder).GetMethod("ToString", Type.EmptyTypes);
}